...
// --- Small keyset (4 keys) used by v0x00 and v0x01 Vector 1 ---

static const char* KEYSET_4_JSON = R"({
...
// --- Large keyset (64 keys, powers of 2) used by v0x00 and v0x01 Vectors 2/3 ---

static const char* KEYSET_64_JSON = R"({
...
TEST_CASE("Keyset JSON valid 4 keys", "[keyset]") {
    auto ks = json::parse(KEYSET_4_JSON).get<Keyset>();
    REQUIRE(ks.size() == 4);
    REQUIRE(ks.count(1) == 1);
    REQUIRE(ks.count(2) == 1);
    REQUIRE(ks.count(4) == 1);
    REQUIRE(ks.count(8) == 1);
}

TEST_CASE("Keyset JSON valid 64 keys", "[keyset]") {
    auto ks = json::parse(KEYSET_64_JSON).get<Keyset>();
    REQUIRE(ks.size() == 64);
    // First and last amounts are powers of 2
    REQUIRE(ks.count(1) == 1);
    REQUIRE(ks.count(9223372036854775808ULL) == 1);
}

TEST_CASE("Keyset JSON roundtrip", "[keyset]") {
    auto ks = json::parse(KEYSET_4_JSON).get<Keyset>();
    json j = ks;
    auto ks2 = j.get<Keyset>();
    REQUIRE(ks.size() == ks2.size());
    for (const auto& [amount, pubkey] : ks) {
        REQUIRE(ks2.count(amount) == 1);
        REQUIRE(ks2.at(amount) == pubkey);
    }
}

TEST_CASE("Keyset JSON invalid pubkey length throws", "[keyset]") {
    // 34-byte pubkey (68 hex chars instead of 66)
    const char* invalid1 = R"({
      "1":"03a40f20667ed53513075dc51e715ff2046cad64eb68960632269ba7f0210e38bcff"
    })";
    REQUIRE_THROWS_AS(json::parse(invalid1).get<Keyset>(), std::runtime_error);
}

TEST_CASE("Keyset JSON invalid uncompressed pubkey throws", "[keyset]") {
    // Uncompressed pubkey (130 hex chars)
    const char* invalid2 = R"({
    ...
    REQUIRE_THROWS(json::parse(invalid2).get<Keyset>());
}

// ============================================================
// Keyset ID derivation v0x00 (V1 deprecated) — NUT-02 spec
// ============================================================

TEST_CASE("Keyset ID v0x00 - 4 keys", "[keyset]") {
    auto ks = json::parse(KEYSET_4_JSON).get<Keyset>();
    auto id = ks.get_keyset_id(0x00);
    REQUIRE(id.to_string() == "00456a94ab4e1c46");
}

TEST_CASE("Keyset ID v0x00 - 64 keys", "[keyset]") {
    auto ks = json::parse(KEYSET_64_JSON).get<Keyset>();
    auto id = ks.get_keyset_id(0x00);
    REQUIRE(id.to_string() == "000f01df73ea149a");
}

// ============================================================
// Keyset ID derivation v0x01 (V2 current) — NUT-02 spec
// ============================================================

TEST_CASE("Keyset ID v0x01 - Vector 1 (4 keys, fee=100, expiry)", "[keyset]") {
    auto ks = json::parse(KEYSET_4_JSON).get<Keyset>();
    auto id = ks.get_keyset_id(0x01, "sat", 100ULL, "2059210353");
    REQUIRE(id.to_string() == "015ba18a8adcd02e715a58358eb618da4a4b3791151a4bee5e968bb88406ccf76a");
}

TEST_CASE("Keyset ID v0x01 - Vector 2 (64 keys, fee=0, expiry)", "[keyset]") {
    auto ks = json::parse(KEYSET_64_JSON).get<Keyset>();
    auto id = ks.get_keyset_id(0x01, "sat", 0ULL, "2059210353");
    REQUIRE(id.to_string() == "01ab6aa4ff30390da34986d84be5274b48ad7a74265d791095bfc39f4098d9764f");
}

TEST_CASE("Keyset ID v0x01 - Vector 3 (64 keys, fee=0, no expiry)", "[keyset]") {
    auto ks = json::parse(KEYSET_64_JSON).get<Keyset>();
    auto id = ks.get_keyset_id(0x01, "sat", 0ULL);
    REQUIRE(id.to_string() == "012fbb01a4e200c76df911eeba3b8fe1831202914b24664f4bccbd25852a6708f8");
}
...
TEST_CASE("Keyset verify_keyset_id v0x00", "[keyset]") {
    auto ks = json::parse(KEYSET_4_JSON).get<Keyset>();
    KeysetId expected{"00456a94ab4e1c46"};
    REQUIRE(ks.verify_keyset_id(expected));
}

TEST_CASE("Keyset verify_keyset_id v0x01", "[keyset]") {
    auto ks = json::parse(KEYSET_4_JSON).get<Keyset>();
    KeysetId expected{"015ba18a8adcd02e715a58358eb618da4a4b3791151a4bee5e968bb88406ccf76a"};
    REQUIRE(ks.verify_keyset_id(expected, "sat", 100ULL, "2059210353"));
}
...
TEST_CASE("Keyset empty throws on get_keyset_id", "[keyset]") {
    Keyset ks;
    REQUIRE_THROWS_AS(ks.get_keyset_id(0x00), std::runtime_error);
}

TEST_CASE("Keyset v0x01 without unit throws", "[keyset]") {
    auto ks = json::parse(KEYSET_4_JSON).get<Keyset>();
    REQUIRE_THROWS_AS(ks.get_keyset_id(0x01), std::runtime_error);
}

TEST_CASE("Keyset unsupported version throws", "[keyset]") {
    auto ks = json::parse(KEYSET_4_JSON).get<Keyset>();
    REQUIRE_THROWS_AS(ks.get_keyset_id(0x02), std::runtime_error);
}
